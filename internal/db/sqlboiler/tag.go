...
var TagTableColumns = struct {
	...
	ID:             "tag.id",
	Name:           "tag.name",
	UpdatedAdminID: "tag.updated_admin_id",
...
var TagWhere = struct {
	...
	UpdatedAdminID: whereHelperstring{field: "\"tag\".\"updated_admin_id\""},
	CreatedAt:      whereHelpertime_Time{field: "\"tag\".\"created_at\""},
	UpdatedAt:      whereHelpertime_Time{field: "\"tag\".\"updated_at\""},
	DeletedAt:      whereHelpernull_Time{field: "\"tag\".\"deleted_at\""},
}
...
var (
	tagAllColumns            = []string{"id", "name", "updated_admin_id", "created_at", "updated_at", "deleted_at"}
	tagColumnsWithoutDefault = []string{"name", "updated_admin_id"}
	tagColumnsWithDefault    = []string{"id", "created_at", "updated_at", "deleted_at"}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(tagPrimaryKeyColumns) == 0 {
				return errors.New("model: unable to upsert tag, could not build conflict column list")
			}

			conflict = make([]string, len(tagPrimaryKeyColumns))
			copy(conflict, tagPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"tag\"", updateOnConflict, ret, update, conflict, insert, opts...)
