...
func New(logger *zerolog.Logger, opts ...Option) *Shutdown {
	shutdown := &Shutdown{
		hooks:               []Hook{},
		mutex:               &sync.Mutex{},
		logger:              logger,
...
loop:
	for i := range hooks {
		...
		s.logger.Info().Msg(fmt.Sprintf("%s is shutting down", hook.Name))
	...
	s.logger.Info().Msg(fmt.Sprintf("time taken for shutdown: %v", time.Since(start)))
